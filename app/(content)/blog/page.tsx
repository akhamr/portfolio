...
async function PostCard({ post }: { post: PostData }) {
  const base64 = await getImage(post.image);
  return (
    <Link className="rounded" href={`/blog/${post.slug}`}>
      <div className="content">
        ...
        <div className="space-y-1 p-2">
          <h4 className="line-clamp-1 leading-snug">{post.title}</h4>
          <div className="text-xs text-muted-foreground md:text-sm">
            {dayjs(post.date).format("MMMM DD, YYYY")}
            {` • ${post.readingTime.text}`}
            {` • ${post.readingTime.words} word(s)`}
          </div>
