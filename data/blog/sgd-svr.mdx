...
This process is repeated until the stopping criteria is met, i.e. $$0 \leq \sum_{i=1}^n (\alpha_i - \alpha_i^*) \leq C$$ degree of tolerance $(tol)$. After that, the optimized $\alpha$ can now be used to calculate the predictive values $(f(\textbf{x}))$.
